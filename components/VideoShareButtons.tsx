...
// Icons copied from SocialShareButtons.tsx for consistency
const FacebookIcon = () => (
...
export const VideoShareButtons: React.FC<VideoShareButtonsProps> = ({ videoBlob, videoUrl, caption }) => {
  ...
  const tryNativeShare = async (): Promise<boolean> => {
    if (!videoBlob || !caption) {
        showMessage('Content not ready to share.');
        return false;
    }
    const file = new File([videoBlob], 'animated-coloring-page.mp4', { type: 'video/mp4' });
    const shareData = {
      files: [file],
      title: 'My Animated Coloring Page!',
      text: caption,
      url: siteUrl,
    };
  ...
  const shareOnFacebook = async () => {
    ...
    const encodedCaption = encodeURIComponent(caption || 'Check out my animated coloring page!');
  ...
  const shareOnX = async () => {
    ...
    const fullText = `${caption || 'My animated coloring page!'} #DogColoringBook #AIArt`;
  ...
  const shareOnInstagram = () => {
    if (!videoUrl || !caption) {
        showMessage('Content not ready.');
        return;
    }
    navigator.clipboard.writeText(caption);
    const link = document.createElement('a');
    link.href = videoUrl;
    link.download = 'animated-coloring-page.mp4';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    
    showMessage(`Caption copied & video saved! Opening Instagram...`, 3500);
  ...
  const shareOnWhatsApp = async () => {
    ...
    const fullText = `${caption || 'My animated coloring page!'} Create your own at ${siteUrl}`;
    const encodedText = encodeURIComponent(fullText);
  ...
  return (
    <div className="w-full relative flex flex-col items-center gap-3">
      ...
      {message && (
        <p className="text-sm font-bold text-teal-700 mt-2 bg-teal-100 px-3 py-1 rounded-md sketch-border border-2 border-teal-300 animate-[fade-in-out_3s_ease-in-out]">
           <style>{`@keyframes fade-in-out { 0%, 100% { opacity: 0; } 10%, 90% { opacity: 1; } }`}</style>
           {message}
        </p>
      )}
    </div>
